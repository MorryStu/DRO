...
AS5X47 as5147p(slaveSelectPin);// Start connection to the sensor.
...
attachInterrupt(digitalPinToInterrupt(clk), clk_ISR, FALLING);
  ...
  tft.setTextSize(1);
  tft.setFont(&FreeMonoBoldOblique12pt7b);
  tft.println(" Ralph the wonder Lama");
  delay(2000);  
  tft.fillScreen(ST77XX_BLACK);
  ...
  tft.fillScreen(ST77XX_BLACK);
  tft.setRotation(1);
  tft.drawRect(0, 0, 320, 46, ST77XX_ORANGE);
  tft.fillRect(0, 50, 320, 46, ST77XX_BLACK);
  tft.fillRect(0, 98,320, 46, ST77XX_BLACK);
  tft.fillRect(0, 147, 320, 46, ST77XX_BLACK);
...
float angle = as5147p.readAngle();
 tft.invertDisplay(false);
  ...
  tft.setTextSize(4);
  if(measured_value<10) tft.print("0");
  if(measured_value<100) tft.print ("0");
  tft.print(measured_value,2);
  ...
  tft.setCursor(20, 60);
  tft.print(measured_value*2,2);
  tft.setCursor(140, 70);
  tft.setFont(&FreeMonoBoldOblique12pt7b);
  tft.setTextSize(1);
  tft.print(" Diam mm");
  tft.setTextColor(ST77XX_AQUA, ST77XX_BLACK);
  tft.setFont();
